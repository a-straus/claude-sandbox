...
| Role | Default | Knobs |
|------|---------|-------|
| Orchestrator iterations (routine) | `opus` at `medium` effort | `ORCH_MODEL`, `ORCH_EFFORT` |
...
| Routine feature workers | `sonnet` at `high` effort | `WORKER_MODEL`, `WORKER_EFFORT` |
| Architecture critics and reviews | `fable` at `high` effort | `ADVISOR_MODEL` |
| Trunk decisions and integration | `opus` at configured effort | owned directly by the orchestrator |
...
worker depth can be raised without changing roles: `WORKER_MODEL=sonnet
WORKER_EFFORT=max`.
